...
class Turbine
  ...
  def set_stages
    ...
    @result.hh.each do |h|
      if @result.hh.index(h) == 0
        a =  self.turbine_stages.build(:p_vh_t => self.p_vh_t, :t_vh_t => self.t_vh_t, :c_vh => @result.c2sr, :h =>h, :ro => 0.3, :phi => 0.97, :psi => 0.97, :alfa1 => 24, :kc => 2.0, :kdc => 0.2, :krk => 3.0, :kdrk => 0.3 )
        TstageResult.new(a,@result)
        a.save
        #self.save
      else
        a = self.turbine_stages.build(:p_vh_t => self.turbine_stages.last.p_vyh_t, :t_vh_t => self.turbine_stages.last.t_vyh_t, :c_vh => @result.c2sr, :h =>h, :ro => 0.3, :phi => 0.97, :psi => 0.97, :alfa1 => 24, :kc => 2.0, :kdc => 0.2, :krk => 3.0, :kdrk => 0.3 )
        TstageResult.new(a,@result)
        a.save
        #self.save
      end
