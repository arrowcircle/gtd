class Property
  ...
  def self.get_alfa(tk)
    alfa = 1.577*0.0000001*tk**2.383+1.774
    return alfa
  end
  
  def self.get_q(lambda,k)
    a = (k+1.0)/2.0
    a = a**(1/(k-1))
  ...
  def self.get_eps(lambda,k)
    b = (1-((k-1)/(k+1))*(lambda**2))
    b = b**(1/(k-1))
    return b
  end
  
  def self.get_tau(lambda,k)
    a = (1-((k-1)/(k+1))*(lambda**2))
    return a
  end
  
  def self.get_a(k,r,t)
    a = sqrt((2*k*r*t)/(k+1))
    return a
  end
  ...
  def self.get_value(current, max, prop)
    case current
    when 1
      return prop*0.79
    when 2
  ...
  def self.get_c_a(current, max, prop)
    case current
    when max
      return (prop - 20.0)
    when max-1
